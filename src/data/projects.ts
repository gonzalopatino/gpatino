...
export const projects: Project[] = [
  {
    ...
    summary: 'Embedded thermostat system with real-time AHT20 sensor reading and threshold-based HVAC control using FreeRTOS.',
    category: 'Embedded Systems',
    problem: 'Need a reliable embedded thermostat with responsive temperature control and local display feedback.',
    solution: 'Built an ESP32 thermostat running FreeRTOS with multi-task architecture: AHT20 sensor reading, threshold-based temperature control, relay actuation for HVAC, and local LCD display for status and manual control.',
    role: 'Solo project. Designed architecture, implemented embedded firmware, and full system testing.',
    stack: ['C/C++', 'FreeRTOS', 'ESP-IDF', 'I2C', 'GPIO'],
    metrics: [
      'Sub-100ms control loop',
      'Reliable AHT20 sensor reading',
      'Watchdog recovery',
    ...
    caseStudy: {
      overview: {
        problem: 'Need a reliable embedded thermostat with responsive temperature control, accurate sensing, and local user interface.',
        users: 'Learning project demonstrating FreeRTOS multi-task architecture and embedded systems design.',
        successCriteria: [
          'Responsive sensor-to-actuation control loop',
          'Accurate temperature/humidity reading from AHT20',
          'Reliable 24/7 operation with watchdog recovery',
          'Clear local display showing system status',
        ],
      },
      constraints: [
        ...
        'Usability: Local display must provide clear feedback on system state',
      ],
      architecture: {
        description: 'Multi-task FreeRTOS architecture with clear separation between sensor acquisition, control logic, actuation, and display.',
        components: [
          { name: 'Sensor Task', description: 'High-priority task reading temperature and humidity from AHT20 sensor via I2C at fixed intervals' },
          { name: 'Control Task', description: 'Threshold-based controller comparing current temperature to setpoint and triggering HVAC actions' },
          { name: 'Actuation Task', description: 'GPIO control of relays for heating, cooling, and fan with safety interlocks' },
          { name: 'Display Task', description: 'Local LCD interface showing current temperature, humidity, setpoint, and system status' },
          { name: 'Watchdog Manager', description: 'System health monitoring with automatic recovery from task failures' },
        ],
      },
      data: {
        source: 'AHT20 temperature and humidity sensor sampled via I2C at configurable intervals.',
        labeling: 'N/A (control system, not ML). Sensor readings validated against physical limits.',
        qualityChecks: [
          'Range validation for physical sensor limits',
          'Noise filtering with smoothing',
          'Sensor fault detection',
          'I2C communication validation',
        ],
        leakagePrevention: 'N/A (real-time control). Focus on sensor calibration.',
      },
      modelOrLogic: {
        baseline: 'Simple on/off control with configurable hysteresis to prevent rapid cycling.',
        iterations: [
          'Implemented basic threshold control with configurable setpoint',
          'Added hysteresis band to reduce relay cycling',
          'Tuned timing parameters for stable operation',
          'Added display feedback for user visibility',
        ],
        whyChosen: 'Threshold-based control with hysteresis is simple, reliable, and well-suited for basic HVAC control. Easy to understand, configure, and debug.',
      },
      evaluation: {
        offlineMetrics: [
          'Control loop execution: < 50ms',
          'Sensor read reliability',
          'Display update responsiveness',
        ],
      ...
      deployment: {
        packaging: 'Firmware built with ESP-IDF, flashed via USB.',
        api: 'Local-only operation. Physical buttons for setpoint adjustment.',
        scaling: 'Single device standalone operation.',
        versioning: 'Semantic versioning for firmware releases.',
        rollback: 'Reflash via USB if needed.',
      },
      safetyPrivacy: {
        piiHandling: 'No personal data collected. Local-only operation.',
        security: [
          'No network connectivity - local operation only',
          'Hardware watchdog for fault recovery',
          'Safe relay control with interlocks',
        ],
      },
      results: [
        ...
        'Clear local display with real-time status updates',
      ],
      nextSteps: [
        'Add WiFi connectivity and MQTT for remote monitoring',
        'Implement scheduling for time-based temperature control',
        'Add occupancy detection for energy optimization',
        'Build web dashboard for remote configuration',
      ],
    },
  },
  {
